...
class QDsim:
    ...
    def init_torchtensor(self):
        ...
        self.Hmatrix_tensor=Hmatvec.BatchedMatVecPreprocessor(self.GPU_cores)
        self.Hmatrix_tensor.transfer_hmatrix(self.local_blocks,len(self.eleVec))
        
        print(f"turely assigned for single GPU: {torch.cuda.memory_allocated() / 1024**3:.2f} GiB")
        print(f"PyTorch reserved for single GPU: {torch.cuda.memory_reserved() / 1024**3:.2f} GiB")
        print(f"Peak assigned for single GPU: {torch.cuda.max_memory_allocated() / 1024**3:.2f} GiB")
        
        #if(rank<self.size_gpu):
    ...
    def RungeKutte_solve_Dormand_Prince_(self,h):
        ...
        # rel_state = np.abs(state_yhk_err / (state_yhk8 + eps))
        # rel_Tt1o  = np.abs(Tt1o_yhk_err  / (Tt1o_yhk8  + eps))
        # rel_Tt2o  = np.abs(Tt2o_yhk_err  / (Tt2o_yhk8  + eps))

        # self.Relerrormax1 = np.max(rel_state)
        # self.Relerrormax2 = max(np.max(rel_Tt1o), np.max(rel_Tt2o))
