...
class QDsim:
    ...
    def init_torchtensor(self):
        ...
        # if(rank<self.GPU_cores):
        #     device = torch.device(f"cuda:{rank}")
        #     torch.cuda.set_device(device)

        #     print(f"Rank {rank}: 绑定 GPU {rank}")

        #     # 2. 初始化 torch.distributed（NCCL）
        #     dist.init_process_group(
        #         backend='nccl',
        #         rank=rank,
        #         world_size=self.GPU_cores
        #     )

        # active_group2 = comm.group.Incl(list(range(self.GPU_cores)))
        # self.comm_gpu = comm.Create_group(active_group2)
        # self.size_gpu=self.GPU_cores
        # self.local_index=[]
        if(rank==0):
            self.Tno_tensor = torch.from_numpy(self.Tno).to(self.device)
            self.Tt_tensor = torch.from_numpy(self.Tt).to(self.device)
            self.Tt1o_tensor=torch.from_numpy(self.Tt1o).to(self.device)
            self.Tt2o_tensor=torch.from_numpy(self.Tt2o).to(self.device)
            self.state_tensor=torch.from_numpy(self.state).to(self.device)
            self.rake_tensor=torch.from_numpy(self.rake).to(self.device)
            self.rake0_tensor=torch.from_numpy(self.rake0).to(self.device)
            self.P_tensor=torch.from_numpy(self.P).to(self.device)
            self.dPdt0_tensor=torch.from_numpy(self.dPdt0).to(self.device)
            #print('self.Tt2o',np.min(self.Tt2o),np.max(self.Tt2o))
            self.a_tensor=torch.from_numpy(self.a).to(self.device)
            self.b_tensor=torch.from_numpy(self.b).to(self.device)
            self.dc_tensor=torch.from_numpy(self.dc).to(self.device)
            self.f0_tensor=torch.from_numpy(self.f0).to(self.device)
            self.slipv_tensor=torch.from_numpy(self.slipv).to(self.device)
            self.slipv1_tensor=torch.from_numpy(self.slipv1).to(self.device)
            self.slipv2_tensor=torch.from_numpy(self.slipv2).to(self.device)
            self.slipvC_tensor=torch.from_numpy(self.slipvC).to(self.device)
            self.shear_loading_tensor=torch.from_numpy(self.shear_loading).to(self.device)
            self.normal_loading_tensor=torch.from_numpy(self.normal_loading).to(self.device)

            #self.vec_Tra_tensor=torch.from_numpy(self.vec_Tra).to(self.device)
            self.fric_tensor=torch.from_numpy(self.fric).to(self.device)
            self.state_tensor=torch.from_numpy(self.state).to(self.device)

            
            self.Hmatrix_tensor=Hmatvec.BatchedMatVecPreprocessor()
            self.Hmatrix_tensor.transfer_hmatrix(self.blocks_to_process,len(self.eleVec))

        
            # self.local_index=self.parallel_cells_scatter_send(nsize=self.size_gpu)


            # local_index=np.array(self.local_index)
            # self.local_index_tensor=torch.from_numpy(local_index).to(self.device)
            ...
            # self.counts = self.comm_gpu.gather(len(self.local_index_tensor), root=0)
            # self.displs = self.comm_gpu.gather(self.local_index_tensor[0], root=0)

            # self.Tno_local_tensor=torch.from_numpy(self.Tno_local).to(self.device)
            # self.Tt1o_local_tensor=torch.from_numpy(self.Tt1o_local).to(self.device)
            # self.Tt2o_local_tensor=torch.from_numpy(self.Tt2o_local).to(self.device)
            # self.state_local_tensor=torch.from_numpy(self.state_local).to(self.device)
        
        #dist.init_process_group(backend='nccl', rank=rank, world_size=self.GPU_cores)
        ...
        if(rank==0):
            ...
            for i in range(totaloutputsteps):
                ...
                if(rank==0):
                    ...
                    print('iteration:',i, flush=True)
                    print('dt:',dttry,' max_vel:',torch.max(torch.abs(self.slipv_tensor)).item(),' min_vel:',torch.min(torch.abs(self.slipv_tensor)).item(),' Seconds:',self.time,'  Days:',self.time/3600/24,
                    'year',year, flush=True)
    ...
    def simu_forward_mpi_tensor(self,dttry):
        slipv1=self.slipv1_tensor-self.slipvC_tensor*torch.cos(self.rake0_tensor)
        slipv2=self.slipv2_tensor-self.slipvC_tensor*torch.sin(self.rake0_tensor)
        #Calculating Kv first
        t0 = MPI.Wtime()
        
        if(self.fix_Tn==True):
            self.dsigmadt=self.normal_loading_tensor
        else:

            self.dsigmadt=self.Hmatrix_tensor.hmatrix_macvec(slipv1,type='Bs')+\
                    self.Hmatrix_tensor.hmatrix_macvec(slipv2,type='Bs')
        self.AdotV1=self.Hmatrix_tensor.hmatrix_macvec(slipv1,type='A1s')+\
                self.Hmatrix_tensor.hmatrix_macvec(slipv2,type='A1d')
        self.AdotV2=self.Hmatrix_tensor.hmatrix_macvec(slipv1,type='A2s')+\
                self.Hmatrix_tensor.hmatrix_macvec(slipv2,type='A2d')
        #print(AdotV1,AdotV2)
        t1 = MPI.Wtime()
        ...
        # t0 = MPI.Wtime()
        # ad1=self.AdotV1.cpu().numpy()
        # ad2=self.AdotV2.cpu().numpy()
        # ad3=self.dsigmadt.cpu().numpy()
        # t1 = MPI.Wtime()
        # self.comm_time += (t1 - t0)
        #Combine results from all ranks
        

        # self.dsigmadt=self.comm_gpu.allreduce(dsigmadt, op=MPI.SUM)
        # self.AdotV1=self.comm_gpu.allreduce(AdotV1, op=MPI.SUM)
        # self.AdotV2=self.comm_gpu.allreduce(AdotV2, op=MPI.SUM)
        ...
        Tno_yhk.clamp_(min=0.1)
        self.Tno_tensor=Tno_yhk
        self.Tt1o_tensor=Tt1o_yhk
        self.Tt2o_tensor=Tt2o_yhk
        self.state_tensor=state_yhk


        self.slipv1_tensor=(2.0*self.V0)*torch.exp(-self.state_tensor/self.a_tensor)*torch.sinh(self.Tt1o_tensor/(self.Tno_tensor-self.P_tensor*1e-6)/self.a_tensor)
        self.slipv2_tensor=(2.0*self.V0)*torch.exp(-self.state_tensor/self.a_tensor)*torch.sinh(self.Tt2o_tensor/(self.Tno_tensor-self.P_tensor*1e-6)/self.a_tensor)
        #print(np.max(np.exp(-self.state_local/self.a[self.local_index])),rank)
        
        self.slipv_tensor=torch.sqrt(self.slipv1_tensor*self.slipv1_tensor+self.slipv2_tensor*self.slipv2_tensor)

        #print(np.max(self.slipv))
        #self.rake=np.arctan2(self.Tt2o,self.Tt1o)
        
        indexmin = torch.where(self.slipv_tensor < 1e-30)[0]

        if len(indexmin) > 0:
            self.slipv_tensor[indexmin] = 1e-30
    ...
    def parallel_cells_scatter_send(self,nsize):
        N=len(self.eleVec)
        ...
        if rank == 0:
            ...
            for i in range(1, nsize):
                self.comm_gpu.send(task_chunks[i], dest=i, tag=77)
            local_index = task_chunks[0] 
        elif(rank<nsize):
            #Non-zero process receiving tasks
            local_index = self.comm_gpu.recv(source=0, tag=77)
    ...
    def derivative_(self,Tno,Tt1o,Tt2o,state):
        ...
        P=self.P_tensor
        dPdt=self.dPdt0_tensor
        AdotV1=self.AdotV1
        AdotV2=self.AdotV2
        shear_loading=self.shear_loading_tensor
        dsigmadt=self.dsigmadt
        slipv=self.slipv_tensor

        def safe_exp(x, max_value=700.0):
            """
            Safe exponential function using torch to prevent overflow.
            Clips the input to [-max_value, max_value] before applying exp.
            
            Args:
                x: Input tensor or value convertible to tensor
                max_value: Maximum absolute value allowed before clipping (default: 700.0)
            
            Returns:
                torch.Tensor: exp(x) with safe clipping
            """
                                 # Ensure input is a torch tensor
            x_clipped = torch.clamp(x, -max_value, max_value)
            return torch.exp(x_clipped)


        def safe_cosh(x, max_value=700.0):
            """
            Safe hyperbolic cosine using exponential form to avoid overflow.
            Clips input and computes cosh(x) = (exp(x) + exp(-x)) / 2.
            ...
            Returns:
                torch.Tensor: safe cosh(x)
            """
            
            x_clipped = torch.clamp(x, -max_value, max_value)
            exp_pos = torch.exp(x_clipped)
            exp_neg = torch.exp(-x_clipped)
            return (exp_pos + exp_neg) / 2


        def safe_sinh(x, max_value=700.0):
            """
            Safe hyperbolic sine using exponential form to avoid overflow.
            Clips input and computes sinh(x) = (exp(x) - exp(-x)) / 2.
        ...
        return dsigmadt*1e-6,dtau1dt*1e-6,dtau2dt*1e-6,dstatedt
    ...
    def RungeKutte_solve_Dormand_Prince_(self,h):
        ...
        Tno=self.Tno_tensor
        Tt1o=self.Tt1o_tensor
        Tt2o=self.Tt2o_tensor
        state=self.state_tensor
        #P=self.P

        
        dsigmadt1,dtau1dt1,dtau2dt1,dstatedt1=self.derivative_(Tno,Tt1o,Tt2o,state)
        ...
        dsigmadt2,dtau1dt2,dtau2dt2,dstatedt2=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        dsigmadt3,dtau1dt3,dtau2dt3,dstatedt3=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        dsigmadt4,dtau1dt4,dtau2dt4,dstatedt4=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        dsigmadt5,dtau1dt5,dtau2dt5,dstatedt5=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        dsigmadt6,dtau1dt6,dtau2dt6,dstatedt6=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        dsigmadt7,dtau1dt7,dtau2dt7,dstatedt7=self.derivative_(Tno_yhk,Tt1o_yhk,Tt2o_yhk,state_yhk)
        ...
        state_yhk_err  = torch.abs(state_yhk8 - state_yhk)
        Tno_yhk_err    = torch.abs(Tno_yhk8   - Tno_yhk)
        Tt1o_yhk_err   = torch.abs(Tt1o_yhk8  - Tt1o_yhk)
        Tt2o_yhk_err   = torch.abs(Tt2o_yhk8  - Tt2o_yhk)

        eps = 1e-10

        rel_state = torch.abs(state_yhk_err / (state_yhk8 + eps))
        rel_Tt1o  = torch.abs(Tt1o_yhk_err  / (Tt1o_yhk8  + eps))
        rel_Tt2o  = torch.abs(Tt2o_yhk_err  / (Tt2o_yhk8  + eps))

        
        self.Relerrormax1 = torch.max(rel_state).item() + eps
        rel1=torch.max(rel_Tt1o).item()
        rel2=torch.max(rel_Tt2o).item()
        self.Relerrormax2 = max(rel1,rel2) + eps
